$(document).ready( function() {
	...
	$('.inspiration-getter').submit(function(event) {
		// clears the results that have been displayed from previous queries
		$('.results').html('');

		var aTags = $("input[name='answerers']").val();
		getAnswerers(aTags);
	})
});
...
var getAnswerers = function(aTags) {
	...
	.done(function(result) {
		...
		for (var i = 0; i < result.items.length; i++) {
			var answer = showAnswerer(result.items[i].user);
...
var showAnswerer = function(answer) {
	...
	var result = $('.templates .answerer-data').clone();

	var answerer = result.find('.answerer');
	answerer.text(answer.display_name);

	var reputationPoints = result.find('.reputation');
	reputationPoints.text(answer.reputation);

	var moreInfo = result.find('.more-info a');
	moreInfo.attr('href', answer.link);
